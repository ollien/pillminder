defmodule Pillminder.Auth do
  ...
  def token_valid_for_pillminder?(token, pillminder) do
    case TokenAuthenticator.token_data(token) do
      :invalid_token -> false
      %{pillminder: :all} -> true
  ...
  def make_token(pillminder) do
    ...
    case TokenAuthenticator.put_token(token, pillminder) do
      :ok -> {:ok, token}
  ...
  def make_single_use_token(pillminder) do
    ...
    case TokenAuthenticator.put_single_use_token(token, pillminder) do
      :ok -> {:ok, token}
      {:error, reason} -> {:error, {:token_store, reason}}
    end
  end
